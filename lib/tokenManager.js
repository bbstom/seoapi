    ...
    async getUserTokens(username) {
        try {
            const [rows] = await pool.query(
                ...
                node_strategy, load_balance_strategy, load_balance_nodes, 
            ...
            return rows.map(row => {
                // 安全解析 JSON 字段
                let loadBalanceNodes = null;
                if (row.load_balance_nodes) {
                    try {
                        loadBalanceNodes = JSON.parse(row.load_balance_nodes);
                    } catch (e) {
                        console.error(`解析 load_balance_nodes 失败 (token ${row.id}):`, e.message);
                ...
                if (row.allowed_models) {
                    try {
                        ...
                        console.error(`解析 allowed_models 失败 (token ${row.id}):`, e.message);
                        allowedModels = null;
                    }
                }
        ...
        try {
            ...
            const [result] = await pool.query(
                ...
                [
                    ...
                    allowedModels && allowedModels.length > 0 ? JSON.stringify(allowedModels) : null,
                    defaultModel || null,
                    fixedNodeId, 
                    fixedModel || null, 
                    tokenId, 
