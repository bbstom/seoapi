...
基于 Node.js + Express 的智能文本改写服务，支持多种 AI 服务和改写模式，兼容小旋风伪原创 API。
...
### 核心功能
- ✅ **多 AI 服务支持** - 自动识别并兼容 7+ 种 AI 服务
  - OpenAI / Azure OpenAI / 国内中转
  - Anthropic Claude 官方
  - Google Gemini
  - Cohere
  - 阿里云通义千问
  - 百度文心一言
- ✅ **智能识别** - 多层识别策略，支持 10+ 个国内中转平台
- ✅ **自动适配** - 即使未知 URL 也能自动识别（默认 OpenAI 格式）
- ✅ **手动指定** - 支持手动指定 API 类型（适用于特殊情况）
- ✅ **统一接口** - 所有 AI 服务使用统一的调用方式
- ✅ **多种改写模式** - 标准、创意、正式、简化、翻译等13+种模式
- ✅ **反 AI 检测** - 专门优化的仿人类写作模式

### 管理功能
- ✅ **用户认证系统** - 每个用户独立的 API Key 和 Claude API Key
- ✅ **Web 管理界面** - 现代化的可视化配置和管理界面
...
- ✅ **多种 API 格式** - 支持 OpenAI、Anthropic、Gemini 等格式
...
详细配置请查看：[AI服务快速配置.md](./AI服务快速配置.md)
...
- [小旋风API接口文档.md](./小旋风API接口文档.md) - 完整 API 参数说明 ⭐

### AI 服务配置
- [AI服务快速配置.md](./AI服务快速配置.md) - 7+ 种 AI 服务配置指南 ⭐
- [AI接口兼容说明.md](./AI接口兼容说明.md) - AI 接口兼容性详细说明
- [增强的API识别机制说明.md](./增强的API识别机制说明.md) - 多层识别策略详解 ⭐
- [手动API类型选择功能说明.md](./手动API类型选择功能说明.md) - 手动指定 API 类型
- [API类型选择快速参考.md](./API类型选择快速参考.md) - 快速参考卡片
...
### Q7: 我的 API 地址不在识别列表中怎么办？

**A:** 不用担心！系统采用多层识别策略：
1. 如果包含 `/v1` 路径，自动识别为 OpenAI 格式
2. 如果完全未知，默认使用 OpenAI 格式（国内中转平台通用）
3. 如果还是不行，可以手动指定 API 类型

详细说明：[增强的API识别机制说明.md](./增强的API识别机制说明.md)
...
即使是未知的中转平台，也会默认使用 OpenAI 格式（通用性最好）。
